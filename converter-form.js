...
(function () {
  ...
  if (typeof ConverterForm._multiMsg !== 'string') {
    ConverterForm._multiMsg = 'Some words you have entered have multiple pronunciations in english. These differences are seperated on a new line';
  }
  ...
  //   ConverterForm.convert = function (inID, outID, errID, type = "IPA") {
  ...
  //         var IPAWord;
  //         if(type.valueOf() === "ETY"){
  //           var IPAWord = VocabList.lookup(englishTextArray[i].toLowerCase().replace(/[^\w\s]|_/g, '').replace(/\s+/g, ' '));
  //         } else{
  ...
  //         ConverterForm._updateParagraph(outID, IPAText);
  //       } else {
  ...
  if (typeof ConverterForm.convert !== 'function') {
    ConverterForm.convert = function (inputWord, outID, errID, type = "IPA") {
      ...
      // We want TextToIPA to exist, or we can't lookup words
        ...
        var IPAWord;
        if(type.valueOf() === "ETY"){
          var IPAWord = VocabList.lookup(inputWord);
        } else{
          var IPAWord = TextToIPA.lookup(inputWord);
        }
